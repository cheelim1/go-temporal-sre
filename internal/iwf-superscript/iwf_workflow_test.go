...
func (s *IWFWorkflowTestSuite) TestIdempotency() {
	...
	runID1, err := s.TestClient.StartWorkflow(context.Background(), SinglePaymentWorkflowType,
		iwf.WorkflowOptions{
			WorkflowID:            workflowID,
			TaskQueue:             TaskQueueName,
	...
	_, err = s.TestClient.StartWorkflow(context.Background(), SinglePaymentWorkflowType,
		iwf.WorkflowOptions{
			WorkflowID:            workflowID,
			TaskQueue:             TaskQueueName,
			WorkflowIdReusePolicy: iwf.WorkflowIdReusePolicyRejectDuplicate,
