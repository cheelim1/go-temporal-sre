...
func (s *WorkflowIdempotencyTestSuite) SetupTest() {
	...
	// Add a short delay between tests to allow Temporal server to fully process
	// any pending activities and workflows
	time.Sleep(500 * time.Millisecond)
}
...
// Delay helper function to use in our workflow for testing concurrency
func addTestDelay(ctx workflow.Context) {
	// Log that we're adding a delay for testing concurrency
	workflow.GetLogger(ctx).Info("Adding test delay in workflow to help demonstrate concurrency behavior")

	// Sleep for a bit to ensure the workflow is open long enough for concurrent tests to detect it
...
func (s *WorkflowIdempotencyTestSuite) TestConcurrentWorkflowIdempotency() {
	...
	options := client.StartWorkflowOptions{
		...
		WorkflowTaskTimeout: 10 * time.Second,
		// This is the key setting - it makes sure temporal will reject duplicate workflow IDs
	...
	for i := 0; i < clientCount; i++ {
		...
		go func(id int) {
			...
			if err == nil {
				...
				// We got an error - check if it's the expected idempotency error
				errorText := err.Error()
				
				// Check both by type assertion and by error message content
				alreadyStartedErr, isAlreadyStarted := err.(*serviceerror.WorkflowExecutionAlreadyStarted)
				containsAlreadyStarted := strings.Contains(errorText, "already started") || 
					strings.Contains(errorText, "WORKFLOW_EXECUTION_ALREADY_STARTED") ||
					strings.Contains(errorText, "WorkflowExecutionAlreadyStarted")
				
				if isAlreadyStarted || containsAlreadyStarted {
					// This is the error we expect when the workflow is already running with this ID
					if isAlreadyStarted {
						s.T().Logf("Client %d received WorkflowExecutionAlreadyStarted - RunID: %s", id, alreadyStartedErr.RunId)
					} else {
						s.T().Logf("Client %d received already started error: %v", id, err)
					}
				} else {
	...
	// Now check the workflow execution using the API
	desc, err := s.client.DescribeWorkflowExecution(ctx, workflowID, "")
	s.NoError(err, "Failed to describe workflow execution")
	
	// Verify that there's exactly one workflow run with this ID
	s.T().Logf("Server reports workflow execution - WorkflowID: %s, RunID: %s, Status: %s", 
	...
	s.T().Logf("SUCCESS: Activity execution count = %d, confirming idempotent execution", deductionCount)
	
	// Wait for the workflow to complete and verify the result
	if len(executions) > 0 {
		s.T().Log("Waiting for the workflow to complete...")
		var result batch.FeeDeductionWorkflowResult
		err := executions[0].Get(ctx, &result)
		s.NoError(err, "Failed to get workflow result")
		s.T().Logf("Workflow result from first client: %+v", result)
		...
		// This is a key feature of Temporal's idempotency model
		...
		s.Equal(result, clientResult, "All clients should get the same result")
		
		// If we received 5 workflow handles but only had 1 activity execution,
		// then this is proof that Temporal's idempotency is working correctly
		s.T().Log("SUCCESS: Temporal's idempotency has been demonstrated:")
		s.T().Logf("1. All %d clients received workflow handles with the SAME RunID", len(executions))
		s.T().Logf("2. Only ONE activity execution occurred (DeductFeeActivity count: %d)", deductionCount)
		s.T().Log("3. Any client can retrieve the identical workflow result")
	} else {
