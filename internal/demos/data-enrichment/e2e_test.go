...
func (a *E2ETestActivities) StoreEnrichedData(enriched data_enrichment.EnrichedCustomer) error {
	a.StoreCount++
	fmt.Println("StoreEnrichedData activity called with customer ID:", enriched.Customer.ID)
	...
	// If configured to fail for a specific customer ID
	if a.FailOnCustomerID != "" && enriched.Customer.ID == a.FailOnCustomerID {
		fmt.Printf("Simulating StoreEnrichedData failure for customer: %s\n", enriched.Customer.ID)
		return fmt.Errorf("simulated storage error for customer: %s", enriched.Customer.ID)
	}
...
func setupE2ETestEnvironment(t *testing.T) (*testsuite.DevServer, client.Client, string, *E2ETestActivities, func()) {
	...
	// Create worker using our centralized worker
...
func TestE2EDataEnrichmentSingleCustomer(t *testing.T) {
	...
	assert.Equal(t, testCustomer.Name, result.Customer.Name, "Customer name should match")
	assert.Equal(t, testCustomer.Email, result.Customer.Email, "Customer email should match")
	assert.Equal(t, 30, result.Demographics.Age, "Demographics age should be set")
	assert.Equal(t, "Test Location", result.Demographics.Location, "Demographics location should be set")
...
func TestE2EDataEnrichmentMultipleCustomers(t *testing.T) {
	...
	// Verify the result
	assert.Equal(t, customer.ID, result.Customer.ID, "Customer ID should match")
	assert.Equal(t, customer.Name, result.Customer.Name, "Customer name should match")
	assert.Equal(t, customer.Email, result.Customer.Email, "Customer email should match")
	assert.Equal(t, 30, result.Demographics.Age, "Demographics age should be set")
	assert.Equal(t, "Test Location", result.Demographics.Location, "Demographics location should be set")
